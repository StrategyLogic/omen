...
def test_load_llm_config_falls_back_to_env_without_file(monkeypatch):
    ...
    config = load_llm_config(Path("/tmp/not-exists-llm-config.toml"), require_embeddings=True)
...
def test_load_llm_config_chat_only_without_voyage(monkeypatch):
    ...
    config = load_llm_config(Path("/tmp/not-exists-llm-config.toml"), require_embeddings=False)
...
def test_load_llm_config_requires_deepseek_key_when_file_missing(monkeypatch):
    ...
    with pytest.raises(FileNotFoundError):
        load_llm_config(Path("/tmp/not-exists-llm-config.toml"), require_embeddings=False)
